...
for player in ivalues(PlayerNumber) do
	...
	local itembox = Def.ActorFrame{
		...
		OnCommand=function(s)
			s:xy( player == PLAYER_1 and SCREEN_CENTER_X-320+74 or SCREEN_CENTER_X+320-74, SCREEN_CENTER_Y+18 )
		end;
	};
	...
	itembox[#itembox+1] = Def.BitmapText{
		...
		InitCommand=function(s)
			...
			s:settext(PlayerName):y( -18-140):zoom(0.7):diffuse( PlayerColor(player) )
			:shadowlength(2)
		end;
	}
	...
	for i,v in ipairs(DifficultyBasedItems) do
		itembox[#itembox+1] = Def.BitmapText{
			...
			OnCommand=function(s)
				s:diffuse( DifficultyColor( "Difficulty_"..v ) ):zoom(0.5)
				:xy( -30,(41-18)+20*(i-1))
			end;
		}
		...
		itembox[#itembox+1] = Def.BitmapText{
			...
			OnCommand=function(s)
				s:diffuse( PlayerColor(player) ):zoom(0.5)
	...
	for i,v in ipairs(CardData) do
		itembox[#itembox+1] = Def.ActorFrame{
			...
			Def.BitmapText{
				...
				Text=v[2],
				OnCommand=function(s)
					s:diffuse( PlayerColor(player) ):zoom(0.7):y(20)
				end;
			};
		}
	end
