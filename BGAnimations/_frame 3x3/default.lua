...
local Args=...
local File,Width,Height=Args.File,Args.Width,Args.Height
...
t[#t+1] = LoadActor( THEME:GetPathB("","_frame files 3x3/"..File) )..{ InitCommand=function(self) self:setstate(3):pause():zoomtoheight(Height*1.73):horizalign(right):x(-Width/2) end; };
t[#t+1] = LoadActor( THEME:GetPathB("","_frame files 3x3/"..File) )..{ InitCommand=function(self) self:setstate(4):pause():zoomtoheight(Height*1.73):zoomtowidth(Width) end; };
t[#t+1] = LoadActor( THEME:GetPathB("","_frame files 3x3/"..File) )..{ InitCommand=function(self) self:setstate(5):pause():zoomtoheight(Height*1.73):horizalign(left):x(Width/2) end; };
